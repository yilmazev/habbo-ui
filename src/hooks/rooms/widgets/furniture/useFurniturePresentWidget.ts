...
const useFurniturePresentWidgetState = () => {
  ...
  useRoomEngineEvent<RoomEngineTriggerWidgetEvent>(RoomEngineTriggerWidgetEvent.REQUEST_PRESENT, event => {
    ...
    setClassId(-1)
    setText((roomObject.model.getValue(RoomObjectVariable.FURNITURE_DATA) || ""))
    setIsOwnerOfFurniture(IsOwnerOfFurniture(roomObject))
    setSenderName((roomObject.model.getValue(RoomObjectVariable.FURNITURE_PURCHASER_NAME) || null))
    setSenderFigure((roomObject.model.getValue(RoomObjectVariable.FURNITURE_PURCHASER_FIGURE) || null))
  })
