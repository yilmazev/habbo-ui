      ...
      items.push(
        <div className="illumina-card-item relative flex h-[122px] flex-col justify-between overflow-hidden p-1.5">
          <button className="flex h-20 items-center" onClick={event => wearFigureAtIndex(index)} disabled={(clubLevel > GetClubMemberLevel())}>
            <LayoutAvatarImage className="wardrobe-avatar !w-full !bg-[center_0] [image-rendering:initial]" figure={figureContainer?.getFigureString()} gender={gender} direction={4} scale={0.7} />
          </button>
          {!hcDisabled && (clubLevel > 0) && <i className="absolute right-1 top-1 h-[9px] w-2.5 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-298px_-69px]" />}
