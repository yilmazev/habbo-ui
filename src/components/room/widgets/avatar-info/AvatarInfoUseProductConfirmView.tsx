...
import { Button, IlluminaCard, IlluminaCardContent, IlluminaCardHeader, LayoutPetImageView } from "../../../../common"
...
export const AvatarInfoUseProductConfirmView: FC<AvatarInfoUseProductConfirmViewProps> = props => {
  ...
  const getPetImage = useMemo(() => {
    ...
    switch (furniData.specialType) {
      ...
      case FurniCategory.MONSTERPLANT_FERTILIZE: {
        ...
        if (roomObject) {
          posture = roomObject.model.getValue(RoomObjectVariable.FIGURE_POSTURE)
  ...
  return (
    <IlluminaCard uniqueKey="use-product-confirm" className="illumina-use-product-confirm w-[350px]">
      <IlluminaCardHeader headerText={LocalizeText("useproduct.widget.title", ["name"], [petData.name])} onCloseClick={onClose} />
