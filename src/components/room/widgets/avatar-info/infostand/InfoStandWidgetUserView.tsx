    ...
    <div className="illumina-card relative w-[206px] px-2.5 pb-3.5 pt-1.5">
      ...
      <div className="flex gap-[9px]">
        <div className="illumina-previewer flex w-[74px] cursor-pointer items-center justify-center" onClick={event => GetUserProfile(avatarInfo.webID)}>
          <LayoutAvatarImage figure={avatarInfo.figure} direction={2} />
        </div>
      ...
      <div className="illumina-card-item h-8 w-full px-1.5">
        <div className="flex h-full items-center gap-2">
          <i className="size-[17px] bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-335px_0px]" />
          {!isEditingMotto &&
          ...
          {isEditingMotto &&
            <textarea className="size-full bg-transparent py-1 text-sm italic !leading-3 [text-shadow:_0_1px_0_#fff] dark:[text-shadow:_0_1px_0_#33312B]" spellCheck={false} maxLength={GetConfiguration("motto.max.length", 38)} onChange={event => setMotto(event.target.value)} onBlur={onMottoBlur} onKeyDown={onMottoKeyDown} >{motto}</textarea>}
        </div>
      </div>
