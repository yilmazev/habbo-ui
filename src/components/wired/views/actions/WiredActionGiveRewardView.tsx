  ...
  return (
    <WiredActionBase requiresFurni={WiredFurniType.STUFF_SELECTION_OPTION_NONE} hasSpecialInput={true} save={save}>
      <div className="flex items-center gap-[14px]">
        <input type="checkbox" id="limitEnabled" onChange={(e) => setLimitEnabled(e.target.checked)} />
        <label className="font-bold">{LocalizeText("wiredfurni.params.prizelimit", [ "amount" ], [ limitEnabled ? rewardsLimit.toString() : "" ])}</label>
      </div>
      {!limitEnabled &&
      ...
      <div>
        <p className="font-bold">How often can a user be rewarded?</p>
        <div className="flex flex-wrap">
          <div className="mb-2 flex justify-between gap-[26px]">
            ...
            <input type="number" className="!h-[13px] w-full border border-black bg-transparent px-px py-0.5" value={limitationInterval} onChange={(event) => setLimitationInterval(Number(event.target.value))} />
          </div>
          <div className="flex">
            <div className="flex gap-2">
      ...
      <div>
        <div className="flex items-center gap-[14px]">
          ...
          <p className="font-bold">Unique rewards</p>
        </div>
