      ...
      <div className={`flex items-center justify-between p-2 ${!isGuide ? "bg-white dark:bg-[#33312b]" : ""}`}>
        ...
        {!isGuide &&
          <div className="flex gap-2">
            <i className="block h-[30px] w-8 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-271px_-252px]" />
            <div>
      ...
      <div className="illumina-scrollbar h-[212px] pt-4">
        {messageGroups.map((group, index) => (
          <div key={index} className={`mb-4 ml-2.5 flex gap-[9px] ${isOwnChat(group.userId) ? "justify-end" : "justify-start"}`}>
            {(isOwnChat(group.userId)) && <LayoutAvatarImage className="!h-[50px] !w-10 shrink-0 !bg-[center_-30px]" figure={GetSessionDataManager().figure} direction={2} />}
            ...
            {(!isOwnChat(group.userId)) && <LayoutAvatarImage className="!h-[50px] !w-10 shrink-0 !bg-[center_-30px]" figure={userFigure} direction={4} />}
          </div>
        ))}
        <div ref={scrollDiv} />
      </div>
