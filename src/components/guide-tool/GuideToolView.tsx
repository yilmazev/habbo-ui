...
export const GuideToolView: FC<{}> = props => {
  ...
  return (
    <IlluminaCard uniqueKey="guide-tool" className="illumina-guide-tool w-[280px]" windowPosition={DraggableWindowPosition.TOP_LEFT}>
      <IlluminaCardHeader headerText={headerText} onCloseClick={event => processAction("close")} noCloseButton={noCloseButton} />
      <IlluminaCardContent className={`text-black ${[GuideSessionState.GUIDE_ONGOING, GuideSessionState.USER_ONGOING].includes(sessionState) || (sessionState === GuideSessionState.USER_FEEDBACK) || (sessionState === GuideSessionState.GUIDE_ACCEPT) ? "!px-px" : ""}`}>
