...
export const FriendsMessengerThreadGroup: FC<{ thread: MessengerThread, group: MessengerThreadChatGroup }> = props => {
        ...
        <div key={index} className="w-full">
          {(chat.type === MessengerThreadChat.SECURITY_NOTIFICATION) &&
            <div className="illumina-input mb-2 flex gap-4 py-2.5 pl-[13px] pr-1.5">
              <i className="mt-1 block h-[21px] w-[23px] shrink-0 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[0px_-153px]" />
          ...
          {(chat.type === MessengerThreadChat.ROOM_INVITE) &&
            <div className="illumina-input relative mb-2 flex py-2.5 pl-[13px]">
              <LayoutAvatarImage className="!absolute -top-3 left-[-5px] !size-[70px] shrink-0 !bg-[center_-15px] drop-shadow-[0px_1px_0_#fff] [image-rendering:initial] dark:drop-shadow-[0px_1px_0_#33312B]" figure={thread.participant.figure} headOnly={true} direction={2} scale={0.50} />
  ...
  return (
    <div className={`mb-4 flex gap-[9px] last:mb-0 ${isOwnChat ? "justify-start" : "justify-end"}`}>
      {isOwnChat && <LayoutAvatarImage className="!h-[50px] !w-10 shrink-0 !bg-[center_-30px]" figure={GetSessionDataManager().figure} direction={2} />}
      ...
      {((group.type === MessengerGroupType.PRIVATE_CHAT) && !isOwnChat) &&
        <LayoutAvatarImage className="!h-[50px] !w-10 shrink-0 !bg-[center_-30px]" figure={thread.participant.figure} direction={4} />}
      {(groupChatData && !isOwnChat) &&
        <LayoutAvatarImage className="!h-[50px] !w-10 shrink-0 !bg-[center_-30px]" figure={groupChatData.figure} direction={4} />}
    </div>
  )
}
