            ...
            {visibleThreads && (visibleThreads.length > 0) && visibleThreads.map(thread => (
              <div key={thread.threadId} className={`size-[35px] overflow-hidden ${activeThread === thread ? "illumina-previewer" : ""}`} onClick={event => setActiveThreadId(thread.threadId)}>
                <div className="relative flex !size-[35px] h-full cursor-pointer items-center justify-center overflow-hidden">
                  {thread.unread &&
                    <i className="absolute right-[3px] top-[7px] z-[1] h-[11px] w-[13px] bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-255px_-70px]" />}
                  {(thread.participant.id > 0) &&
                    <LayoutAvatarImage className="!size-[70px] shrink-0 !bg-[center_-18px] drop-shadow-[0px_1px_0_#fff] [image-rendering:initial] dark:drop-shadow-[0px_1px_0_#33312B]" figure={thread.participant.figure} headOnly={true} direction={2} scale={0.50} />}
                  {(thread.participant.id <= 0) &&
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-1">
                  ...
                  <Button className="!h-5 !px-[7px]" onClick={openProfile}>
                    <i className="block h-3 w-4 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-436px_0px]" />
                  </Button>
