...
import { LayoutAvatarImage, LayoutBadgeImageView } from "../../../../../common"
...
export const FriendsListGroupItemView: FC<FriendsListGroupItemViewProps> = props => {
    ...
    <div className="illumina-card-item relative flex h-[26px] items-center pr-2.5">
      <div className="flex h-[26px] flex-1 items-center">
        {searchFriend.isAvatarOnline || isFriend
          ? <LayoutAvatarImage className={`!h-[52px] !w-8 !bg-[center_-25px] [image-rendering:initial] ${selected ? "drop-shadow-[0px_1px_0_#33312B]" : "drop-shadow-[0px_1px_0_#fff] dark:drop-shadow-[0px_1px_0_#33312B]"}`} figure={searchFriend.avatarFigure} headOnly={true} direction={3} scale={0.50} />
          : <div className="h-[52px] w-8" />}
        <div className="flex items-center gap-1">
          <div className="cursor-pointer" onClick={event => GetUserProfile(searchFriend.avatarId)}>
            <i className={`mt-px block h-3 w-4 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] ${selected ? "bg-[-436px_-13px]" : "bg-[-436px_0px]"}`} />
          </div>
      ...
      <div className="flex items-center gap-2">
        {isFriend &&
          <div className="cursor-pointer" onClick={openMessengerChat(searchFriend.avatarId)}>
            <i className={`block size-3.5 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] ${selected ? "bg-[-242px_-100px]" : "bg-[-180px_-98px]"}`} />
          </div>}
        {canRequestFriend(searchFriend.avatarId) && !isFriend &&
          <div className="cursor-pointer" onClick={event => requestFriend(searchFriend.avatarId, searchFriend.avatarName)}>
            <i className="block size-4 bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-197px_-100px]" />
          </div>}
      </div>
    </div>
  )
  ...
  if (request && !isSearch) return (
    <div className="illumina-card-item relative flex h-[26px] items-center pr-2.5">
      <div className="flex h-[26px] flex-1 items-center">
        {request.id > 0 && <LayoutAvatarImage className="!h-[52px] !w-8 !bg-[center_-25px] drop-shadow-[0px_1px_0_#fff] [image-rendering:initial] dark:drop-shadow-[0px_1px_0_#33312B]" figure={request.figureString} headOnly={true} direction={3} scale={0.50} />}
        <div className="flex items-center gap-1">
      ...
      <div className="flex items-center gap-2">
        <div className="nitro-friends-spritesheet icon-accept cursor-pointer" onClick={event => requestResponse(request.id, true)}>
          <i className="block h-3.5 w-4 cursor-pointer bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-180px_-83px]" />
        </div>
        <div className="nitro-friends-spritesheet icon-deny cursor-pointer" onClick={event => requestResponse(request.id, false)}>
          <i className="block size-[13px] cursor-pointer bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-226px_-100px]" />
        </div>
      </div>
    </div>
  )
  ...
  if (friend && !isSearch) return (
    <div className={`illumina-card-item relative flex h-[26px] cursor-pointer items-center pr-2.5 ${selected ? "dark" : ""}`} onClick={event => selectFriend(friend.id)}>
      <div className="flex h-[26px] flex-1 items-center">
        {friend.id > 0 && friend.online && <LayoutAvatarImage className={`!h-[52px] !w-8 !bg-[center_-25px] [image-rendering:initial] ${selected ? "drop-shadow-[0px_1px_0_#33312B]" : "drop-shadow-[0px_1px_0_#fff] dark:drop-shadow-[0px_1px_0_#33312B]"}`} figure={friend.figure} headOnly={true} direction={3} scale={0.50} />}
      ...
      <div className="flex items-center gap-2">
        <i className={`h-3.5 w-[13px] bg-[url('/client-assets/images/spritesheet.png?v=2451779')] ${getCurrentRelationshipName()}`} />
        <div className="cursor-pointer" onClick={openRelationship}>
          <i className="block h-[5px] w-[9px] bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-282px_-23px]" />
        </div>
        ...
        {friend.followingAllowed && <>
          ...
          <div className="cursor-pointer" onClick={clickFollowFriend}>
            <i className={`block h-2.5 w-[9px] bg-[url('/client-assets/images/spritesheet.png?v=2451779')] ${selected ? "bg-[-453px_-11px]" : "bg-[-453px_0px]"}`} />
          </div>
        </>}
      </div>
      {isRelationshipOpen &&
        <div ref={relationshipRef} className="illumina-card-item absolute right-[60px] top-5 z-10 flex flex-col items-center px-1 py-2">
          ...
          <div className="h-3.5 w-[13px] cursor-pointer bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-292px_-23px]" onClick={event => clickUpdateRelationship(event, MessengerFriend.RELATIONSHIP_HEART)} />
          <div className="h-3.5 w-[13px] cursor-pointer bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-320px_-23px]" onClick={event => clickUpdateRelationship(event, MessengerFriend.RELATIONSHIP_SMILE)} />
          <div className="h-3.5 w-[13px] cursor-pointer bg-[url('/client-assets/images/spritesheet.png?v=2451779')] bg-[-306px_-23px]" onClick={event => clickUpdateRelationship(event, MessengerFriend.RELATIONSHIP_BOBBA)} />
        </div>}
    </div>
  )
